...
function PatientDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([getPatientDocuments(patient.databaseId), getPatientConsentRequests(patient.databaseId), getPatientIntakes(patient.databaseId)])
      .then(([docs, consentRequests, intakeRecords]) => { setDocuments(docs); setRequests(consentRequests); setIntakes(intakeRecords) })
...
function HealthHistory({ intakes }: { intakes: PatientIntakeRecord[] }) {
  return <section className="mt-6 space-y-5">{intakes.length === 0 ? <div className="rounded-3xl border border-slate-200 bg-white p-12 text-center text-sm text-slate-500">No clinical questionnaire has been completed yet.</div> : intakes.map(intake => {
    const normalizedAnswers = (Array.isArray(intake.answers) ? intake.answers : [])
      .map((answer, index) => {
        const questionText = typeof answer?.question === 'string' && answer.question.trim()
          ? answer.question
          : typeof answer?.question_text === 'string' && answer.question_text.trim()
        ...
        return { question: questionText, answer: answerText, key: `${intake.id}-${questionText}-${index}` }
      })
      .filter(pair => pair.question && pair.answer)

    return <article key={intake.id} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-teal-700">Clinical questionnaire</p><h2 className="mt-1 text-xl font-black">{intake.clinical_summary?.chiefComplaint || 'Patient intake'}</h2></div><div className="text-right text-xs text-slate-500"><p>{new Date(intake.created_at).toLocaleString()}</p><p className="mt-1 capitalize">{intake.history_mode} mode</p></div></div>{intake.is_emergency && <div className="mt-4 rounded-xl bg-rose-50 p-3 text-sm font-bold text-rose-700">Emergency signs were flagged during this intake.</div>}{normalizedAnswers.length > 0 ? <div className="mt-5 space-y-3">{normalizedAnswers.map((answer, index) => <div key={answer.key} className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-bold uppercase tracking-wider text-slate-500">Question {index + 1}</p><p className="mt-1 font-semibold text-slate-800">{answer.question}</p><p className="mt-2 text-sm text-teal-800">{answer.answer}</p></div>)}</div> : <div className="mt-5 rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">No questionnaire answers were stored for this intake yet.</div>}</article>
  })}</section>
}
